...
/**
 * Atomically apply balance changes within a caller-supplied transaction.
 *
 * This is the single authoritative balance-mutation path for the wallet. It
 * performs non-negativity checks, a version-locked (`updateMany`) wallet
 * update, and emits matching append-only ledger entries. All reward/economy
...
export async function executeBalanceChange(args: ExecuteBalanceChangeArgs) {
  ...
  return prisma.$transaction(async (tx) =>
    applyBalanceChanges(tx, userId, changes, { idempotencyKey, operationName })
  );
}
